...
def get_client_secrets(client_id: str, project_id: str = None) -> dict:
    """
    Fetch all 4 secrets for a given client from Secret Manager.

    For client_id='default', fetches the platform owner's tokens.
    For any other client_id, fetches their registered credentials.

    Returns a dict with keys:
        github_token, jenkins_token, jenkins_url, jenkins_user
    """
    if not project_id:
